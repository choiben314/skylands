...
public class EnemyIslandManager {
	...
	public EnemyIsland makeRandomEnemyIsland() {
		...
		EnemyIsland enemy = new EnemyIsland(300, 700, imageName, 7);
		...
		for (int i = 0; i < gunPos.length - 1; i++) {
			if (gunPos[i] == 1) {
				Gun gNew = new Gun(Island.FIRST_BUILDING_POS[0] + 42 * i, Island.FIRST_BUILDING_POS[1], ImagePaths.GUN1_42x42, false, 10);
				eb.add(gNew);
			} else {
				ResourceProducer rNew = new ResourceProducer(Island.FIRST_BUILDING_POS[0] + 42 * i, Island.FIRST_BUILDING_POS[1], ImagePaths.BUILDING_TEST, false, "wood");
				eb.add(rNew);
			}
		}
		return enemy;
